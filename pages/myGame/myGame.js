...
Page({
  ...
  setCurrentArr: function(nameStr) {
    ...
    gameArr =
      this.data.activeIndex === "0" || this.data.activeIndex === 0
        ? this.data.availableGames
        : this.data.activeIndex === "1"
        ? this.data.myCreatedGames
        : this.data.myEnroledGames;
    ...
    if (gameArr) {
      filtered = gameArr.filter(e => {
        ...
        return (
          e.gameName.indexOf(nameStr) >= 0 ||
          e.gamePublisher.name.indexOf(nameStr) >= 0
        );
      });
    }
    ...
    if (
      filtered.length > 0 &&
      (this.data.activeIndex === "2" || this.data.activeIndex === 2)
    ) {
      for (let i = 0; i < filtered.length; i++) {
        if (filtered[i].referees) {
          filtered[i].assigned =
            filtered[i].referees.filter(
              r => r.openid === app.globalData.openid && r.assigned
            ).length > 0;
        }
      }
    }
